...
fn parse_sleep_seconds(subcommand_args: Vec<String>, subcommand_argv: HashMap<String, Vec<String>>) -> Option<FixedPrec> {
  if subcommand_args.len() >= 1 {
    return Some(FixedPrec::from_str(&subcommand_args[0], 9));
  }
  
  let sleep_seconds_option = subcommand_argv.get("time-seconds").and_then(|v| v.last());
  match sleep_seconds_option {
    Some(x) => return Some(FixedPrec::from_str(x, 9)),
    None => {},
  }
  ...
  match sleep_milliseconds_option {
    Some(x) => {
      ...
      return Some(match sleep_milliseconds {
  ...
  match sleep_microseconds_option {
    Some(x) => {
      ...
      return Some(match sleep_microseconds {
        FPNumber { integer_part, fractional_part, .. } => FPNumber {
          negative: false,
  ...
  match sleep_nanoseconds_option {
    Some(x) => {
      ...
      return Some(match sleep_nanoseconds {
